...
try {
  for (const width of [320, 390, 834, 1024]) {
    const page = await browser.newPage({
      viewport: { width, height: 844 }, hasTouch: true,
    ...
    for (const route of ["/rexonance-saga", "/extreme-saga"]) {
      ...
      const layout = await page.locator(".rxs-local-nav-inner").evaluate((inner) => {
        ...
        return { gap: edge.right - menu.right, clear: brand.right < menu.left, size: menu.width, overflow: document.documentElement.scrollWidth - innerWidth };
      });
      assert.ok(Math.abs(layout.gap) <= 1 && layout.clear && layout.size >= 48 && layout.overflow <= 1, JSON.stringify(layout));
      await checkSelect(page.locator(".rxs-comparison-selector select"), page);
      if (route === "/rexonance-saga") await checkSelect(page.locator(".rxs-p14-native-select select"), page);
      const tabs = page.locator('.rxs-stage-tabs [role="tab"]');
      await tabs.first().focus();
      await page.keyboard.press("ArrowRight");
      await page.waitForFunction(() => document.querySelectorAll('.rxs-stage-tabs [role="tab"]')[1]?.getAttribute("aria-selected") === "true");
      ...
      const semantics = await page.locator('.rxs-stage-panel[role="tabpanel"]').evaluate((panel) => {
        const label = document.getElementById(panel.getAttribute("aria-labelledby"));
        return label?.getAttribute("aria-selected") === "true" && label?.getAttribute("aria-controls") === panel.id && label?.tabIndex === 0;
      });
      assert.equal(semantics, true);
      console.log(`PASS ${width}px ${route}: header alignment, keyboard/pointer selection, labelled tabs`);
    }
    await page.goto(base + "/riders/saga");
    const links = await page.locator(".dossier-reader-links a").evaluateAll((items) => items.map((item) => ({ height: item.getBoundingClientRect().height, font: parseFloat(getComputedStyle(item).fontSize) })));
    ...
    for (const kind of ["saga", "realm"]) {
      await page.waitForFunction(() => document.querySelector("#archive-switcher")?.getAttribute("aria-busy") === "false");
      ...
      assert.ok(rect.width >= 47.9 && rect.height >= 47.9, `${kind} home target: ${JSON.stringify(rect)}`);
    }
