    ...
    try {
      for (const [route, selector] of cases) {
        ...
        for (let step = 1; step <= 10; step++) {
          await cdp.send("Input.dispatchTouchEvent", {
            type: "touchMove",
            touchPoints: [{ x: point.x, y: point.y - (distance * step) / 10 }],
          });
        ...
        const state = await heading.evaluate((node) => {
          ...
          return {
            y: scrollY,
            selectionCancelled: selection.defaultPrevented,
            copyBlocked: copy.defaultPrevented,
            overflow: document.documentElement.scrollWidth > innerWidth + 1,
            nativePan: getComputedStyle(document.body).touchAction,
            scansIgnoreTouch: [
              ...document.querySelectorAll(".film-text-scan, .film-text-scan > i"),
            ].every((element) => getComputedStyle(element).pointerEvents === "none"),
          };
        });
        assert.ok(
          state.y - before > 45,
          `${width} ${route} ${selector}: pan stalled ${JSON.stringify(state)}`,
        );
        ...
        console.log(
          JSON.stringify({
            ...
            selector,
            hit: point.hit,
            scrollDelta: Math.round(state.y - before),
          }),
        );
      }
    } finally {
