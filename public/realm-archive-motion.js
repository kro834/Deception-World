...
(() => {
  ...
  const archiveNav = navLinks[0]?.closest('.archive-nav');
  const navEntries = navLinks.map((link, index) => {
    const target = document.getElementById(link.hash.slice(1));
    link.dataset.locationIndex = String(index + 1).padStart(2, '0');
    return target ? { link, target, index } : null;
  }).filter(Boolean);
  let activeLocationIndex = -1;
  let locationIntentIndex = -1;
  let locationIntentTimer = 0;

  function setArchiveLocation(entry) {
    if (!entry) return;
    navEntries.forEach(item => {
      item.link.setAttribute('aria-current', String(item.index === entry.index));
    });
    root.dataset.archiveLocation = entry.target.id;
    root.style.setProperty('--archive-location-progress', String((entry.index + 1) / Math.max(navEntries.length, 1)));
    archiveNav?.setAttribute('data-current-location', `${String(entry.index + 1).padStart(2, '0')}/${String(navEntries.length).padStart(2, '0')}`);
    if (activeLocationIndex === entry.index) return;
    activeLocationIndex = entry.index;
    if (archiveNav && archiveNav.scrollWidth > archiveNav.clientWidth) {
      const left = entry.link.offsetLeft - (archiveNav.clientWidth - entry.link.offsetWidth) / 2;
      archiveNav.scrollTo({ left: Math.max(0, left), behavior: 'auto' });
    }
  }

  function releaseArchiveLocationIntent() {
    if (locationIntentIndex < 0) return;
    locationIntentIndex = -1;
    window.clearTimeout(locationIntentTimer);
    locationIntentTimer = 0;
    schedulePageUpdate();
  }

  function holdArchiveLocationIntent(entry, delay = 1400) {
    locationIntentIndex = entry.index;
  ...
  function resolveArchiveLocationEntry(measured, probe, atDocumentEnd, currentIndex) {
    const byDocumentTop = [...measured].sort((a, b) => a.top - b.top || a.index - b.index);
    if (!byDocumentTop.length) return null;
    if (atDocumentEnd) return byDocumentTop[byDocumentTop.length - 1];

    const formEntry = measured.find(entry => entry.index === 0);
    const detailEntry = measured.find(entry => entry.index === 1);
    const reachedEntries = byDocumentTop.filter(entry => entry.top <= probe + 1);
    let current = reachedEntries[reachedEntries.length - 1] || formEntry || byDocumentTop[0];
    const introRowIsClustered = Boolean(
      formEntry
      && detailEntry
      && Math.abs(formEntry.top - detailEntry.top) <= 96
    );
    const laterSectionReached = byDocumentTop.some(entry => entry.index > 1 && entry.top <= probe + 1);
    if (introRowIsClustered && !laterSectionReached) {
      // On iPad the two intro columns share one visual row, but their heading
      // rectangles can cross by a few pixels. Keep an explicit Form/Detail
      // choice stable; when returning from a later section, default to Form.
      const preferredIntroIndex = currentIndex === 0 || currentIndex === 1 ? currentIndex : 0;
      current = preferredIntroIndex === 1 ? detailEntry : formEntry;
    }
    return current;
  }
  ...
  function updateArchiveLocation() {
    ...
    const intended = navEntries.find(entry => entry.index === locationIntentIndex);
    if (intended) {
      setArchiveLocation(intended);
      return;
    }
    const scrollTop = window.scrollY || document.documentElement.scrollTop || 0;
    const probe = scrollTop + Math.min(240, Math.max(96, window.innerHeight * 0.18));
    const measured = navEntries.map(entry => ({
      ...entry,
      top: entry.target.getBoundingClientRect().top + scrollTop
    }));
    const maxScroll = Math.max(0, document.documentElement.scrollHeight - window.innerHeight);
    const atDocumentEnd = maxScroll > 0 && scrollTop >= maxScroll - 2;
    const current = resolveArchiveLocationEntry(measured, probe, atDocumentEnd, activeLocationIndex);
    setArchiveLocation(current);
  }

  navEntries.forEach(entry => {
    entry.link.addEventListener('click', () => holdArchiveLocationIntent(entry));
  });
  ...
  window.addEventListener('scroll', () => {
    schedulePageUpdate();
    if (locationIntentIndex >= 0) {
      window.clearTimeout(locationIntentTimer);
      locationIntentTimer = window.setTimeout(releaseArchiveLocationIntent, 220);
    }
  }, { passive: true });
  ...
  const locationResizeObserver = 'ResizeObserver' in window
    ...
    : null;
  locationResizeObserver?.observe(root);
