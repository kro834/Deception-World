...
type Transaction = <T>(callback: (sql: Sql) => Promise<T>) => Promise<T>;
...
function toSql(run: Run, transaction?: Transaction): Sql {
  ...
  sql.transaction =
    transaction ??
    (async <T>(callback: (transactionSql: Sql) => Promise<T>) => callback(sql));
  return sql;
}
...
function createNeonSql(): Promise<Sql> {
  globalRef.__pgSqlPromise__ ??= (async () => {
    ...
    return toSql(run, async <T>(callback: (transactionSql: Sql) => Promise<T>) => {
      const client = await pool.connect();
      try {
        await client.query("BEGIN");
        const transactionRun: Run = async <Row>(text: string, params: unknown[]) => {
          const result = await client.query(text, params);
          return result.rows as Row[];
        };
        const transactionSql = toSql(transactionRun);
        const value = await callback(transactionSql);
        ...
        try {
          ...
          // Preserve the original database error when the connection is gone.
        }
        throw error;
      } finally {
        client.release();
...
async function createPgliteSql(): Promise<Sql> {
  ...
  return toSql(run, async <T>(callback: (transactionSql: Sql) => Promise<T>) =>
    pg.transaction(async (transaction) => {
      ...
      return callback(toSql(transactionRun));
    }),
  );
}
