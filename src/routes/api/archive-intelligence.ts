...
import {
  ...
  startArchiveIntelligenceAiRequest,
...
import { archiveIntelligenceRequestSchema } from "@/lib/archive-intelligence.server";
import {
  ...
  server: {
    handlers: {
      POST: async ({ request }) => {
        ...
        try {
          const state = await startArchiveIntelligenceAiRequest(
            request,
            parsed.data,
