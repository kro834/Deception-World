...
One WebGL fragment shader (`rising.frag.glsl`, high precision where the GPU has it) draws the dive and the burn within a pixel budget: phones that fit it (412 × 915 is 377k of 420k CSS px) start at full CSS resolution, so the print is as sharp as the portal it takes over from; larger screens start at about three quarters. It draws every frame on 60 and 90 Hz panels and every second frame on 120 and 144 Hz panels, and steps down a resolution-first quality ladder only when frames run late; its last two rungs drop whole layers (the smoke's detail, the ash, the haze, the second spark layer and the embers, then the smoke, the sparks and the crack breaks), so they buy GPU time on weak devices. The engine is loaded with `import()` when the gate nears the viewport. Its images are prepared as resized ImageBitmaps, and the GL context and shader compile start at pointerdown (for touch, 60 ms into the contact or at pointerup: a flick the browser takes over before then (pointercancel) creates none; a finger that rests on the button before scrolling can still prime one, which is released after 1 s unless the click adopts it). Until the engine has loaded, the dialog stays dark. The context exists only while the sequence plays and is released at the end, on close and on pagehide. The prepared bitmaps are closed when the gate unmounts and prepared again on the next approach.
...
- behind it the char shrinks into plates: a crack net whose edges curl up grey-white, greying to ash where it has burned longest, a ghost of the print in the carbon; the cracks and ember specks glow orange at the front and cool to dull red over a couple of seconds (cooling is counted in seconds since the front passed, not in distance), each patch breathing on its own slow beat, so the char under the title is never a flat void;
- curled sheets of ash lift off the front, a lit grey face, a shadowed back and a burning rim, turning as they rise; small flakes tumble higher up;
- the flames are domain-warped turbulence advected upwards, each tongue swelling and sinking on its own beat, its tip bent furthest by the sway and pinched off into flamelets, coloured by a blackbody ramp: yellow-white only at the hot root, orange in the body, dim deep red at the tips, translucent where thin, with no holes at the root;
- smoke is a lit medium, not a darkener: grey-brown billows with sharp silhouettes rise faster than the front, widen as they climb and collect in the top third, lit warm underneath and self-shadowed on top, lifting the dark road and veiling (never muddying) the armour; heat haze is a narrow, mostly vertical shimmer just above the flames;
...
The dive's zoom blur jitters its taps per pixel (interleaved gradient noise), so it grains instead of ghosting, and the breakthrough bloom multiplies what is lit, so the blacks stay black.

The calm CSS tier burns the same image with raster sprites that only move by transform and opacity, rendered offline from the same fire model by `scripts/render-rising-calm-sprites.mjs` (`public/rising-calm-*-20260924.webp`, about 110 KB, loaded only when the tier shows): a burn-edge strip on a fixed fractal profile (`rising-calm.ts`, low-passed so it tears into tongues and bays instead of zig-zagging) whose incandescent lip varies in width and brightness along it, fresh char behind it with cracks that glow and cool, and a tiled char texture below; three flame frames seated along the lip at irregular sizes and gaps, behind the strip so the char cuts their roots, two frames per seat taking turns while rising a little (fake advection, each seat on its own beat); lit grey-brown smoke billows that swell, drift and thin; and embers whose streaks lean along their wandering paths. Landscape screens deepen the edge's tears. Reduced motion keeps only the cross-fade to the still.
