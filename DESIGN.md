...
- **How.** `src/components/world/reveal-text.tsx` (with `reveal-label.ts`) splits the text into inline `span.tr-c` (Array.from, so server and client always match). The component is memoised, and the heading fragments are module constants in `world-home.tsx`. `src/styles-world-reveal.css` animates only `color` (and the cursor cell's `background-color`) on one named view timeline per block (`--tr`). It loads before Mirage. This adds no layers and keeps line breaking.
