...
### The flashback — 2026-09-24

Between the press and the print burning away, eleven supplied scenes flash back over the dive and the burn, in the order supplied (`public/rising-flashback-*-20260924.webp`, cut to 800 px and graded as a memory by `scripts/build-rising-flashback.mjs`: warm, partly drained, dimmed; the white hall dimmer still). They run from 0.3 s to 5.7 s of the sequence clock (5.4 s on the CSS tier) in a layer above the fire and under the title, so the title cut stays legible. They are gone before the print has burned away.
- **Motion.** Each scene cross-fades into the next and settles from a slight push-in, a new one every 0.42–0.45 s. The layer is capped at 85% opacity, and there are no cuts.
- **Photosensitivity.** The flash audit holds every tier at one flash a second, with no red flashes.
- **Loading.** The scenes are warmed with the engine on approach, never on the page's own load. They are lazy images in the dialog.
...
## Dossier edition (character files) — 2026-09-27

`src/styles-dossier-edition.css` carries the Mirage grammar into the character files: `/riders/*`, `/managers/*` except Zeus, and `/characters/*` (Luna, Terra, Dante, シエル). It is a re-skin only. The layout, the copy and each character's accent (`--manager-accent`) are unchanged, and the sheet adds no text (every `content` is empty). The sovereign file (`.is-sovereign`, Zeus) keeps its own gold design, so every rule is scoped to `main.manager-page:not(.is-sovereign)`.
- **Delivery.** `DOSSIER_STYLESHEET_LINKS` (`src/lib/world-head.ts`) is the World sheets, then the same Michroma HUD subset as `/world` (`DOSSIER_HUD_FONTS_URL`, so a file opened from World reuses it), then this sheet. `createRiderHead` and the manager and character routes use it. Page sheets (`styles-ciel.css`, `styles-dante.css`) load after it and keep their colours.
- **Panels.** Square (`--dossier-radius` is overridden to 4px; the pinned 12px default stays in `styles-dossier-reader.css`) and locked on with brackets: the character's accent (lifted toward ice) across the top, gold along the bottom. This applies to the portrait, quotes, facts, chapter cards, related records, form pickups (not the Rexonance pickup, which keeps its own ornaments), the contents list and the list return. Prism hairlines top the quotes, facts, chapter cards, the reader bar and the pagination.
- **Type.** Chrome-ink names with a static misregistered ghost (`drop-shadow`, off under economy), outlined chapter numerals, and HUD labels in Michroma at 12px or larger. Controls are square plates rather than pills. Forced colours get plain ink.
- **Motion.** Brackets lock on, and numerals and rules step in, on the page's own view timelines. Each finishes at `entry 100%` or earlier and never loops. It is gated like the rest of the dossier choreography (reduced motion, economy, side menu, loading covers, dialogs), and keyframes use only opacity and scale. Panels that clip (portrait, facts, pickups) keep static brackets, because they would capture an anonymous `view()`.

Guards: `scripts/dossier-edition.test.mjs`.
